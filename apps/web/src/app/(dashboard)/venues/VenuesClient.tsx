...
import { Building2, Plus, MoreHorizontal, Pencil, Trash2, Star, MapPin, Users, Phone, Mail, Globe } from "lucide-react"
...
const defaultForm = {
  ...
  rentalFeeMin: "", rentalFeeMax: "", cateringType: "", parkingAvailable: false,
...
export function VenuesClient({ eventId, venues: initialVenues, role }: Props) {
  ...
  function openAdd() { setEditVenue(null); setForm(defaultForm); setDialogOpen(true) }
  ...
  function openEdit(venue: Venue) {
    ...
    setForm({
      ...
      rentalFeeMin: venue.rentalFeeMin ?? "", rentalFeeMax: venue.rentalFeeMax ?? "",
  ...
  async function handleSubmit() {
    ...
    try {
      const actionData = {
        ...
        rentalFeeMin: form.rentalFeeMin || undefined, rentalFeeMax: form.rentalFeeMax || undefined,
      ...
      const stateData: Venue = {
        ...
        rentalFeeMin: form.rentalFeeMin || null, rentalFeeMax: form.rentalFeeMax || null,
      ...
      if (editVenue) {
        ...
        setVenues(vs => vs.map(v => v.id === editVenue.id ? stateData : v))
        toast.success("Venue updated!")
      } else {
        const res = await createVenue(eventId, actionData)
        if (res.error) { toast.error(res.error); return }
        setVenues(vs => [...vs, res.venue as Venue])
        ...
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="grid grid-cols-2 gap-4 py-4">
            ...
            <div className="space-y-2">
              ...
              <Input value={form.rentalFeeMin} onChange={e => setForm({ ...form, rentalFeeMin: e.target.value })} placeholder="50000" />
            </div>
            <div className="space-y-2">
              <Label>Rental Fee Max (₹)</Label>
              <Input value={form.rentalFeeMax} onChange={e => setForm({ ...form, rentalFeeMax: e.target.value })} placeholder="100000" />
            </div>
            <div className="space-y-2">
